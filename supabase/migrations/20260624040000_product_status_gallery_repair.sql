-- Repair product visibility/gallery behavior for production databases that use
-- products.product_status as the product archive/delete source of truth.
...
CREATE POLICY "Public can view active products"
  ...
  USING (
    active = true
    AND coalesce(product_status, 'active') NOT IN ('archived', 'deleted', 'draft', 'hidden', 'inactive')
  );
  ...
  USING (
    EXISTS (
      ...
      WHERE p.id = product_images.product_id
        AND p.active = true
        AND coalesce(p.product_status, 'active') NOT IN ('archived', 'deleted', 'draft', 'hidden', 'inactive')
    )
  );
